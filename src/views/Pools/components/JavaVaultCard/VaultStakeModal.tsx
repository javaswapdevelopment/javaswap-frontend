...
import { fetchJavaVaultUserData } from 'state/pools'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertJavaToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 JAVA or less
    ...
    if (isWithdrawingAll) {
      try {
        const tx = await callWithGasPrice(javaVaultContract, 'withdrawAll', undefined, callOptions)
  ...
  if (showRoiCalculator) {
    return (
      <RoiCalculatorModal
        ...
        stakingTokenBalance={javaAsBigNumber.plus(stakingMax)}
