...
describe('hooks', () => {
  describe('#queryParametersToSwapState', () => {
    ...
    test('output MATIC only', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=matic&exactAmount=20.5', { parseArrays: false, ignoreQueryPrefix: true }),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: 'MATIC' },
        [Field.INPUT]: { currencyId: '' },
    ...
    test('invalid recipient', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=MATIC&exactAmount=20.5&recipient=abc', { parseArrays: false, ignoreQueryPrefix: true }),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: 'MATIC' },
        [Field.INPUT]: { currencyId: '' },
