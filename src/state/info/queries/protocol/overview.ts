...
const formatJavaFactoryResponse = (rawJavaFactory?: JavaFactory) => {
  if (rawJavaFactory) {
    return {
      totalTransactions: parseFloat(rawJavaFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawJavaFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawJavaFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatJavaFactoryResponse(data?.javaFactories?.[0])
      const overviewData24 = formatJavaFactoryResponse(data24?.javaFactories?.[0])
      const overviewData48 = formatJavaFactoryResponse(data48?.javaFactories?.[0])
