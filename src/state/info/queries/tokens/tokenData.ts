...
import { useMaticPrices } from 'views/Info/hooks/useMaticPrices'

interface TokenFields {
  id: string
  symbol: string
  name: string
  derivedMATIC: string // Price in MATIC per token
...
interface FormattedTokenFields
  extends Omit<TokenFields, 'derivedMATIC' | 'derivedUSD' | 'tradeVolumeUSD' | 'totalTransactions' | 'totalLiquidity'> {
  derivedMATIC: number
  derivedUSD: number
...
const parseTokenData = (tokens?: TokenFields[]) => {
  ...
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    const { derivedMATIC, derivedUSD, tradeVolumeUSD, totalTransactions, totalLiquidity } = tokenData
    accum[tokenData.id] = {
      ...tokenData,
      derivedMATIC: parseFloat(derivedMATIC),
...
const useFetchedTokenDatas = (tokenAddresses: string[]): TokenDatas => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedMATIC * maticPrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedMATIC * maticPrices.oneDay : 0
          const priceUSDWeek = week ? week.derivedMATIC * maticPrices.week : 0
    ...
    if (tokenAddresses.length > 0 && allBlocksAvailable && !blockError && maticPrices) {
      fetch()
    }
  }, [tokenAddresses, block24h, block48h, block7d, block14d, blockError, maticPrices])
